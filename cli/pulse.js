...
argv.forEach(async (key, index) => {
  ...
  if (["--port", "-port", "-p"].includes(key)) {
    if (isNaN(value)) return error("Usage: openradio --port [Port Number]");
    ...
    if (!value) return error("Usage: openradio --output-samplerate [num]");
...
server.on("request", (req, res) => {
  ...
  res.writeHead(200, {
    "content-type": config.contenttype || "audio/" + config.output.format,
  });
...
let listener = server.listen(config.server.port, config.server.address, () => {
  ...
  function play() {
    ...
    parec.stdout
      .pipe(pcmconv(config.output, config.input))
      .on("error", (err) => console.error(`[${Date()}]`, err))
      .on("data", (chunk) => {
